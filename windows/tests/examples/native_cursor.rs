...
fn main() {
    ...
    phase(&h, "hidden by ShowCursor from startup", 12);
    // SAFETY: balances the one hide above, restoring this thread's initial display count.
    assert_eq!(unsafe { ShowCursor(1) }, 0);
    phase(&h, "visible native arrow", 4);
    h.set_thread_cursor(0);
    phase(&h, "hidden by SetCursor(NULL)", 12);
    h.set_thread_cursor(arrow);
    phase(&h, "visible native arrow restored", 4);
}

fn phase(h: &Harness, label: &str, seconds: u64) {
