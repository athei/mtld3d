...
command a reader has to be told about. Three options are gated this way today:
`buffer.ignoreLockBounds` in `buffers.rs`, `depth.aliasSameSize` in
`render_target.rs`, and `debug.expandPacked16` in `expand16.rs`.
