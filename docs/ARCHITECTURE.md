...
Render-ahead is what a synchronous read-back pays for. A back-buffer `LockRect`, `GetRenderTargetData` or `GetDC` has to flush the frame so far and then wait for the GPU, and the flush drains every stage in front of it: the encoder's hand-off, then each in-flight `SubmitFrame`, each of which sits in `nextDrawable` for about one display interval because the present throttle holds drawables to the panel cadence. Measured on a game that reads the back buffer back for every newly targeted unit, that is three to four intervals, a 47 ms frame at 120 Hz, against under a millisecond of actual work in the flushed frame. `present.renderAhead = 0` trades the overlap away: `Present` waits until its own frame's `SubmitFrame` has returned (`SubmitGate`, a seq the submit thread publishes), so the next read-back finds nothing queued and waits only for pending GPU work. Because a drawable throttle would then put a full interval after the game's work in every frame, that mode sends the layer "no vsync, no cap" and paces the API thread itself: a vsync request follows the panel's maximum refresh rate, published by the unix side into a `DisplaySinks` word at attach and on every display move, `present.maxFps` caps it, the lower wins, and vsync off with no cap free-runs. The default is unchanged; the WoW app profile opts in.
